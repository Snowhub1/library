...
function Library:Window(Args)
    ...
    local CornerGlowColor   = Args.CornerGlow  
    ...
    Library:Create("UICorner", { Parent = Background })
        ...
        local videoUrl = "https://github.com/Snowhub1/Other/raw/refs/heads/main/Ghost.webm"
    local folder = "hyf_55"
    ...
    local fileName = assetFolder .. "/." .. SanitizeFilename(videoUrl) .. ".webm"
    ...
    task.spawn(function()
        if not isfile(fileName) then
            ...
            if req then
                local response = req({Url = videoUrl, Method = "GET"})
                if response and response.Success then writefile(fileName, response.Body) end
            else
...
function Library:KeySystem(Args)
    ...
    local ShowDiscordTab = tostring(Args.DiscordTabShow):lower() ~= "false"  
